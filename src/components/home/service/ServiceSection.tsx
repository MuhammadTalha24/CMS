...
const ServiceSection: FC<ServiceSectionProps> = ({ ...props }) => {
	...
	const visibleItemsByBreakpoint = useBreakpointValue({
		base: 4,
		xl: 3,
		'2xl': 4,
	});
	...
	const handleToggleView = () => {
		if (isShowingAll) {
			setVisibleItems(visibleItemsByBreakpoint || 4); // Reset to initial state
		} else {
	...
	useEffect(() => {
		if (visibleItemsByBreakpoint) {
			setVisibleItems(visibleItemsByBreakpoint);
		}
